    ...
    <div>
      ...
      <div className="mt-4 flex flex-wrap items-end gap-2 rounded-xl border border-pitch-border bg-pitch-surface p-4">
        <div>
          ...
          <input type="number" value={number} onChange={(e) => setNumber(e.target.value)} className="mt-1 block w-24 rounded-lg border border-pitch-border bg-pitch px-3 py-2 text-sm text-slate-900 outline-none focus:border-slate-900" />
        </div>
        <div>
          <label className="text-xs text-slate-500">Deadline for squad changes</label>
          <input type="datetime-local" value={deadline} onChange={(e) => setDeadline(e.target.value)} className="mt-1 block rounded-lg border border-pitch-border bg-pitch px-3 py-2 text-sm text-slate-900 outline-none focus:border-slate-900" />
        </div>
        ...
        {gameweeks.map((gw) => (
          <div key={gw.id} className="flex flex-wrap items-center justify-between gap-2 px-4 py-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setStatus(gw.id, "locked")} className="flex items-center gap-1 rounded-md bg-amber-500/20 px-2 py-1 text-xs font-bold text-amber-600 hover:bg-amber-500/30">
                <Lock size={12} /> Lock
              </button>
              ...
              <button onClick={() => setStatus(gw.id, "completed")} className="flex items-center gap-1 rounded-md bg-emerald-500/20 px-2 py-1 text-xs font-bold text-emerald-600 hover:bg-emerald-500/30">
      ...
      <div className="mt-8 rounded-xl border border-rose-500/30 bg-rose-500/5 p-4">
        <h2 className="text-sm font-bold text-rose-700">End Season</h2>
        ...
        <button onClick={endSeason} className="mt-3 flex items-center gap-1 rounded-lg bg-rose-500/20 px-4 py-2 text-sm font-bold text-rose-700 hover:bg-rose-500/30">
